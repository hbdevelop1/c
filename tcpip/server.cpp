...
int main(int argc, char *argv[])
{
    ...
    //‘SOCK_STREAM’ the transport layer protocol needed should be reliable ie it should have acknowledgement techniques.
    //For example : TCP
    //0 to let the kernel decide of the default protocol to use for this connection. For reliable connections, the default
    ...
    //(in case the system has multiple interfaces on the network because wired interface has an IP different from the wireless
    //interface)
    ...
    while(1)
    {
        ...
        auto iterator_th = find_if(begin(v_threads), end(v_threads),
                                   [&first_message_done](const auto & th)
                                    {
